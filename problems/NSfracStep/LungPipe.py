...
def pre_solve_hook(velocity_degree, mesh, **NS_namesepace):
    ...
    DG = FunctionSpace(mesh, "DG", 0, constrained_domain=constrained_domain)
    ...
    return dict(uv=uv,n=n,h=h,domains=domains,DG=DG)


def temporal_hook(mesh, u_, dt, h, V, uv, DG, name, n, domains, inlet_ID, outlet_ID, \
					tstep, check_flux, check_CFL, **NS_namespace):
